"""Shared trainer-flow infrastructure (010) — MinIO client, config, and the ephemeral-Prefect shim.
...
the MinIO/MLflow endpoints, the boto3 client built from env-only credentials (no hardcoded default —
...
def s3_client():
    """boto3 S3 client for MinIO. Credentials come from the environment only — no hardcoded default
...
def fetch_jsonl(name: str, version: str):
    """Pull a pinned dataset version from MinIO and yield parsed JSON rows (one per non-blank line).
