...
.DESCRIPTION
  ...
  Steps: compose down -> drop ONLY `mlops-lite_pgdata` (MinIO datasets/artifacts + Grafana survive) ->
  ...
  addressed on MinIO) and are NOT touched.
...
if (-not $Confirm) {
    Write-Host ("DESTRUCTIVE: this drops the MLflow run/trace history (the mlops-lite_pgdata volume). " +
        "MinIO datasets/artifacts are preserved. Re-run with -Confirm to proceed:`n" +
...
# (that would also wipe miniodata = datasets + model artifacts).
docker compose down
...
# the re-seed — register/promote + vision seed only touch MLflow/MinIO/the gateway, not the GPU daemons.
...
try { & "$PSScriptRoot/up_all.ps1" -Distro $Distro }
    ...
    Write-Warning ("up_all reported an issue ($($_.Exception.Message)). This is often a transient daemon " +
        "health flap during bring-up. Proceeding to re-seed (it needs only MLflow/MinIO/gateway); " +
...
Write-Host "`nMLflow 3.x reset complete — fresh backend, registry re-seeded, datasets preserved on MinIO." -ForegroundColor Green
