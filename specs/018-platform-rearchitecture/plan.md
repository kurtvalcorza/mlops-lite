...
| **III. Lightweight Footprint** | ✅ Supervised native daemons ~8 → 2 (+ the shrunken babysitter); `pynvml` is a tiny host-only library; Postgres is **already resident** for MLflow (the `gateway` DB has been provisioned since 001 — adopting it adds no service). Idle RAM budget re-verified on hardware (SC-107). |
