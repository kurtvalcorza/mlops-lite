...
> Scope: add **hyperparameter optimization** to the fine-tune path via **Optuna** (pure-Python,
> server-less) + **MLflow** parent/child runs, objective = **011's eval metric**, **trials strictly
> sequential on the single GPU** (`n_jobs=1`), best trial → registered version (eligible for 011's gated
> promotion) + surfaced in the Runs UI. New dep: `optuna` (light). **GPU/FT stack FROZEN.** No
> constitution amendment (library only, no daemon — Ray Tune rejected). Tasks T222–T237.
>
...
- [ ] **T222** [US1] Confirm `optuna` (latest stable) installs clean in the training venv **without
...
- [ ] **T223** [US1] `training/requirements.txt`: add `optuna`; leave the **frozen** GPU/FT stack
  untouched; record the pin in `scripts/native_env.lock`. (FR-111, FR-118)
- [ ] **T224** [US1] NEW `training/flows/hpo.py`: Optuna study runner — `optuna.create_study(direction=…)`,
...
- [ ] **T225** [US1] MLflow parent/child tracking in `hpo.py`: open a **parent (study) run**
...
- [ ] **T226** [US1] `training/trainer.py`: add `POST /study` (launch — one study at a time on the lease,
...
- [ ] **T227** [US1] `gateway/app/routers/runs.py`: proxy `POST /studies` + `GET /studies/{id}` to the
...
- [ ] **T228** [P] [US1] `test_hpo_study`: a study with `n_trials=3` runs **exactly 3 trials
...
- [ ] **T229** [US2] NEW `training/search_spaces.py`: declare **per-modality** search spaces with
...
- [ ] **T230** [US2] Wire the trial **objective** in `hpo.py` to **011's eval harness**: after
...
- [ ] **T231** [P] [US2] `test_hpo_objective`: an LLM-LoRA study samples within the LLM space (only
...
- [ ] **T232** [US3] In `hpo.py`, on study completion register **only the best trial** as a new MLflow
...
- [ ] **T233** [US3] Confirm the registered best version is a valid candidate for **011's gated
  promotion** (it flows into the existing eval-gated promotion path — same tags/shape the gate expects).
  (FR-116)
- [ ] **T234** [US3] `ui/` Runs tab: add the **minimal HPO surface** — an **"optimize" toggle** on the
...
- [ ] **T235** [P] [US3] `test_hpo_register`: after a study completes, **exactly one** model version
...
- [ ] **T236** [P] No-regression: the existing single-value `POST /runs` fine-tune path, the trainer's
...
- [ ] **T237** Commit `optuna` pin (`requirements.txt` + `native_env.lock`), `flows/hpo.py`,
