...
- [ ] **T781c** `[US1]` `[P]` (**FR-457c**, **FR-458**) The unpinned counterpart: the same stale-alias setup may serve the stale version, and the response must name the version it **served** — never the newly promoted one it did not. Check: request `model` equals response `model` on a stale hit.
