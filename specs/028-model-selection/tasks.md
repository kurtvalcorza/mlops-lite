...
- [ ] **T779a** `[US1]` (**FR-477**, **FR-477a**, **FR-477b**) Define and implement the **authority** for each of the four defaults. Check: only the LLM modality has one today — `registry.DEFAULT_LLM` (`SERVING_MODEL`) with `active_serving_llm_name()`; `registry.resolve_serving_target(task)` documents that when several models share a task *"otherwise the first match is used"*, which is discovery, not a designated default. Assert all four cases per modality: the source of the identity; a **permanent** refusal naming the modality when no default exists; deterministic behaviour when several promoted models carry the modality and none is designated — never an arbitrary first match; and that the resolver's chosen identity is the one the runtime actually answers with, since a divergence here recreates the 022 defect through the default path.
