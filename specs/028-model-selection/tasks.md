...
- [ ] **T779a** `[US1]` (**FR-477**, **FR-477a**, **FR-477b**, **FR-477c**, **FR-477d**, **FR-477e**) Give the per-modality default pointers **routing authority**, per FR-477c. Three already ship and only need promoting from attribution to routing — `SERVING_MODEL`, `ASR_SERVING_MODEL` (`gateway/app/routers/transcribe.py:25`), `VISION_SERVING_MODEL` (`vision.py:24`), the last two already passed as `prefer_name` inside best-effort `_resolve_*_version()` helpers that only label a prediction log. Add **one** new pointer, `EMBED_SERVING_MODEL`, following that convention; do **not** invent a second name for a pointer that exists. Resolve via `registry.resolve_serving_target(task, prefer_name=<pointer>)`. Check, per modality, all five states: (a) **pointer unset, exactly one promoted model carries the task** → still answered, by that model — the back-compat half, and the half that regresses; (b) pointer set and promoted → that identity, and a second promoted model carrying the same task does **not** displace it; (c) **pointer unset, two or more promoted** → **409 `model_default_unconfigured`**, and assert the message names **the pointer to set**, not merely that it refused; (d) pointer set but not promoted for the modality → refused, never served by whatever was found; (e) no promoted serving model at all → the same 409 naming the modality. The *"otherwise the first match is used"* branch is unreachable in (a) by construction — one candidate has no ordering — which is what makes (a) and (c) consistent rather than contradictory; an earlier revision required (a) to work while forbidding the only branch that could have produced it. Do **not** assert that configuration guarantees resolver/runtime agreement: FR-477e establishes that the gateway pointer and the agent-side identity are separate env vars, so the assertion belongs to T783d's compare-and-pin, which **detects** divergence and refuses.
