...
- [ ] **T779a** `[US1]` (**FR-477**, **FR-477a**, **FR-477b**, **FR-477c**, **FR-477d**) Implement the per-modality default **pointers** settled in FR-477c — one per modality, mirroring `SERVING_MODEL`, each **defaulting to the identity that modality’s fixed engine already serves**, resolved via `registry.resolve_serving_target(task, prefer_name=<pointer>)`. Check, per modality, all four cases: (a) with the pointer unset, an omitting request still works and is answered by the engine’s current identity — the back-compat half, and the half that regresses; (b) a second promoted model carrying the same task does **not** displace the pointer’s model, and the *"otherwise the first match is used"* branch is never taken; (c) a pointer naming a model not promoted for the modality is refused rather than served by whatever was found; (d) a modality with **no** promoted serving model is refused **409 `model_default_unconfigured`** naming the modality. Also assert the resolver’s chosen identity is the one the runtime answers with — a divergence here recreates the 022 defect through the default path. The policy is settled in the spec, not here: leaving it to this task would let (d) be implemented for all three non-LLM modalities, which conforms to FR-477b and breaks every omitting caller.
