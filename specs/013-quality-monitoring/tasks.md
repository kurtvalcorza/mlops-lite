...
> **Status (2026-06-29):** **BUILT** (US1–US3). `gateway/app/quality.py` (sibling of `monitoring.py`):
> fire-and-forget/fail-open prediction logging + label ingestion + windowed per-modality quality
> (reuses 011's metric libs) + relative-to-baseline breach + OR-cooldown combine. Wired into
> `routers/infer.py`/`stream.py`/`vision.py` (logging) + `routers/monitor.py` (`POST /monitor/labels`,
> `POST /monitor/quality/check`, `GET /monitor/quality`); Grafana dashboard extended; thin batch label
> client `data/submit_labels.py`. **No heavy deps** (pure-Python + 011 libs), `monitoring.py` untouched,
> GPU/FT stack frozen, no amendment. Tasks T238–T255.
>
> **Build note — decisions (recorded):**
> - **Store layout (T238):** existing MinIO `results` bucket under `predictions/`, `labels/`, `quality/`
>   prefixes (mirroring `drift/`); records keyed by `prediction_id`, tagged with `model_version`. No new
>   datastore.
> - **Prediction logging (T239/T240):** the 006 tracing pattern — synchronous id, background daemon-thread
>   write, bounded semaphore, swallow-all fail-open; capture toggle `QUALITY_CAPTURE_IO`. `/infer` returns
>   the `prediction_id` (additive); `/infer/stream` logs the prompt+id but leaves output uncaptured (SSE
>   stays byte-identical); `/vision/classify` logs the top-1 label + image content-hash ref.
> - **Windowing (T243/T244):** sliding count-based — last `QUALITY_WINDOW_N` labeled pairs, scored only
>   over labeled pairs (unlabeled stay pending), `< QUALITY_MIN_PAIRS` ⇒ *insufficient data* (no false
>   breach). Metric from 011's registry by modality (accuracy/task-accuracy/WER). Pure `score_window`/
>   `evaluate_quality`/`is_breach` split from I/O for offline tests.
> - **Threshold (T247):** relative-to-baseline, **direction-aware** — higher-better breaches on a drop
>   below `baseline*(1-DROP_PCT)`, lower-better (WER) on a rise above `baseline*(1+DROP_PCT)`; baseline =
>   the 011 eval value read from the version's registry tags.
> - **Combine policy (T249):** OR + cooldown. The input-PSI trigger's fire decision is **unchanged**
>   (leading signal — `/monitor/check` only records the retrain timestamp, so the PSI path doesn't
>   regress); the quality trigger (confirming signal) respects the shared in-process cooldown. A new
>   `gateway_retrain_signal_total{signal,result}` counter keeps the two signals independently observable
>   alongside the unchanged `gateway_retrain_triggers_total`.
> - **No second resident model / no latency regression:** quality is CPU-side aggregation off the request
>   path. 19 new offline quality tests (logging/join/window/metric/breach/cooldown); full suite green
>   (`83 passed, 31 skipped` offline).
>
...
- [x] **T238** [US1] Confirm **011's metric libs** import as a library and cover the needed modalities
...
- [x] **T239** [US1] Add `gateway/app/quality.py` (sibling of `monitoring.py`): a **fire-and-forget,
...
- [x] **T240** [US1] Wire prediction logging into `routers/infer.py`, `routers/stream.py` (SSE — frame-safe,
...
- [x] **T241** [US1] Add the **label-ingestion path** to `routers/monitor.py`: the primary primitive is
...
- [x] **T242** [P] [US1] Tests: serve N inferences → N prediction records with unique ids + model version;
...
- [x] **T243** [US2] In `quality.py`, implement **windowed quality compute**: join predictions↔labels over a
...
- [x] **T244** [US2] Add a **`POST /monitor/quality/check`** on-demand recompute endpoint to `monitor.py`
...
- [x] **T245** [US2] **Extend** `infra/grafana/.../mlops-lite.json` with a **model-quality panel** (per
  modality / model version) placed **beside** the existing input-drift panels (do not remove/relayout the
  PSI panels). (FR-124)
- [x] **T246** [P] [US2] Tests: windowed quality over labeled pairs == a hand-computed expected value;
...
- [x] **T247** [US3] Add a per-modality **relative-to-baseline degradation threshold**: mark a window's
  ...
  (FR-125)
- [x] **T248** [US3] On a breach (and if a retrain spec is supplied), fire the **existing** `_launch_retrain`
...
- [x] **T249** [US3] Implement the **combine policy** for the two breach signals — **OR + configurable
...
- [x] **T250** [P] [US3] Tests: below-threshold window ⇒ retrain launched **exactly once** (counter +1,
...
- [x] **T251** [P] Re-validate the **input-PSI** path unchanged: existing drift compute, `gateway_drift_score`
...
- [x] **T252** [P] UI no-regression: the six tabs + the BFF contract (allowlist, origin guard, `[::1]`,
  non-leaky errors, key absent from payloads) unchanged; any quality surfacing is a thin read, not a new
  datastore/tool. (SC-080)
- [x] **T253** [P] Serving no-regression: SSE framing byte-identical; **no second resident model**; the
  one-model-in-VRAM mutex + GPU-lock hold time + inference latency unchanged with logging on. (SC-080)
- [x] **T254** [P] Confirm **no heavy deps** added (no Evidently/pandas/scipy) and the **GPU/FT stack
  frozen** (no torch-family movement); footprint stays within Principle III. (SC-080)
- [x] **T255** Full prior keyed sweep green with model-quality monitoring in place; quality reports +
