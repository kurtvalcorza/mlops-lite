...
> **Status (2026-06-29):** **BUILT** (US1–US3). Hand-rolled validation (`gateway/app/validation.py`)
> gating `finetune_flow` before `train_lora` + advisory `POST /datasets/{name}/{version}/validate`;
> offline batch inference (`gateway/app/batch.py` core + native `training/flows/batch_infer.py` flow +
> trainer `/batch` + gateway `/batch` proxy, content-addressed results); UI wire-in (datasets validation
> report + Runs batch launcher) + BFF allowlist. No new dep/service/stage. Tasks T256–T275.
>
> **Build note — decisions (recorded):**
> - **Validation (T257):** pure `run_rules(rows, …)` (offline-testable) + `validate_dataset` I/O wrapper.
>   Rules: required-columns (any-of groups, e.g. instruction|prompt) + min-rows **hard-gate**; null-rate,
>   value-range, label-balance **warn**. `report["passed"]` reflects only gate rules; all thresholds +
>   dispositions configurable (env defaults). No Great Expectations / no new dep (mirrors PSI/content-
>   addressing precedents).
> - **Training gate (T259):** `validate_dataset_or_raise` runs at the TOP of `finetune_flow` — before
>   `mlflow.start_run()`/`train_lora` — so a gate failure costs no MLflow run and no GPU (`DatasetValidationError`
>   carries the report); warn rules tagged on the run. One `validation.py`, two call sites (gate + route).
> - **Batch (T263/T264):** `run_batch` (record-and-continue per-row, abort > `BATCH_ABORT_THRESHOLD` after
>   `BATCH_ABORT_MIN_ROWS`) + `write_result` (content-addressed sha256 → `results/batch/<job>/<ver>/{data,
>   manifest}`, idempotent). Injectable `predict_fn` (offline tests) / live per-modality serving predictor.
> - **Mechanism (grill A, T265):** native ephemeral Prefect flow `batch_infer.py`, launched via gateway
>   `POST /batch` → trainer `/batch` (mirrors `/runs`↔`/train`). **Lease:** a GPU batch drives the serving
>   supervisor/bento (the single one-model-in-VRAM lease tenant — online `/infer` serialized by it, no 2nd
>   model pinned); tabular off-lease; the daemon `_active` gate serializes batch vs train/study. On-hardware
>   step: the precise acquire-once/hold-in-flow variant (SC-082) is validated on the GPU; tests inject the
>   predictor + a fake store.
> - 21 new offline tests (validation 11 + training-gate 3 + batch 7); pure logic split from I/O. Full
>   suite green offline.
>
> **Firm decisions:**
...
- [x] **T256** [US2] Confirm the serving path can be invoked **row-by-row under a single GPU-lease
...
- [x] **T257** [US2] Implement `gateway/app/validation.py` — **hand-rolled lightweight** checks over a
...
- [x] **T258** [US2] Add `gateway/app/routers/validation.py` — `POST /datasets/{name}/{version}/validate`
...
- [x] **T259** [US2] Gate `training/flows/finetune.py`: in `finetune_flow`, run validation on the
...
- [x] **T260** [US2] Record validation results to MLflow on the **fail-open** basis (006 precedent) —
  MLflow down never blocks the gate or the register-time advisory check. (FR-133)
- [x] **T261** [P] [US2] `tests/test_validation`: clean dataset → `passed=true` + stats; broken datasets
  (empty / missing columns / over-null / under-min-rows) → `passed=false` naming each failed rule
  value-vs-threshold. (SC-083)
- [x] **T262** [P] [US2] `tests/test_finetune_gate`: training launched on a **failing** dataset is
...
- [x] **T263** [US1] Implement `gateway/app/batch.py` — score **every** row of a dataset version through
...
- [x] **T264** [US1] Write the result **content-addressed** to MinIO (sha256 of result bytes, mirroring
...
- [x] **T265** [US1] Wire the **ephemeral Prefect flow** mechanism (grill A): `gateway/app/routers/batch.py`
...
- [x] **T266** [US1] Log the batch run to MLflow (params: dataset version, model, counts; result URI) on
  the **fire-and-forget, fail-open** basis (006/007) — MLflow down never blocks the job. (FR-133)
- [x] **T267** [P] [US1] `tests/test_batch`: one output per input row (count match); result artifact lands
  content-addressed + downloadable with a manifest; terminal status `succeeded`. (SC-081)
- [x] **T268** [P] [US1] `tests/test_batch_lease`: GPU batch acquires the lease **once** + releases at end;
...
- [x] **T269** [US3] Datasets surface: render the per-rule **ValidationReport** inline (pass/fail + stats)
  for a selected dataset version, fetched via the BFF. (FR-134)
- [x] **T270** [US3] Batch launcher + status in the **existing Runs surface** (grill C — no new Batch tab):
  ...
  (FR-134, FR-129)
- [x] **T271** [US3] BFF: **allowlist** the new `/batch*` + `/datasets/*/validate` proxy routes under the
...
- [x] **T272** [P] [US3] `tests/test_ui_batch_validation` (+ extend `test_ui_security`): report renders;
...
- [x] **T273** Full 001–007 keyed sweep green with 014 features **idle**: online `/infer`(+`/stream`),
...
- [x] **T274** Confirm the GPU-lock **hold time** + online `/infer` **latency** are unchanged when batch is
  idle, and that a completed batch job has **released** the VRAM lease (no lingering resident model).
  (SC-082, SC-086)
- [x] **T275** Commit the new modules/routes/tests, the UI wire-in, any `batch-results` bucket
