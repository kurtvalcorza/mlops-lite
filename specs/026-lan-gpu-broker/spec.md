...
- **FR-021**: An interactive session MUST automatically release the GPU after a configurable idle period and MUST be ended at a configurable maximum lifetime (TTL). "Idle" MUST mean *no GPU work*, measured from activity the broker itself admitted — a client-supplied liveness signal (a notebook heartbeat) MUST NOT reset the GPU idle timer, or an abandoned session left open in a browser holds the GPU until its TTL and this requirement does nothing in the case it exists for.
...
- **FR-024**: Admission MUST enforce **both** bounds — accounted set + reservations ≤ usable budget, **and** incoming load ≤ live free VRAM − **not-yet-materialized reservations** − headroom (without that deduction two concurrent admissions can each claim the same free bytes) — and MUST measure the actual VRAM consumed after load **per child process** (a device-wide delta is not attributable when loads overlap), rolling back if an estimate drifted. A model that does not fit MUST be refused with a clear reason rather than failing unsafely, distinguishing a **permanently** oversized model (`model_too_large`) from **transient** contention (`gpu_busy`, retryable).

**Scheduling & isolation**

- **FR-025**: When multiple requests/jobs contend for the GPU, the broker MUST order them by **shape-based lanes with FIFO within each lane** — inference interleaved ahead of exclusive jobs, jobs FIFO — with owner override, and a running job never preempted. GPU ordering MUST have a **single authority** — the host-agent coordinator. Every path that can occupy the GPU MUST enter through it; in particular, policy-triggered retrains (018 `PolicyScheduler`, which has no tenant identity and today calls `/train` directly) MUST enqueue onto the jobs lane under a reserved **system tenant** rather than bypassing it. To prevent job starvation, after a configurable inference burst **or** head-job wait bound the broker MUST enter a **job-drain mode** that stops admitting NEW inference (running requests finish) until the head FIFO job acquires the GPU. The job queue MUST be **persisted** so ordering survives a host-agent restart — persistence alone is not sufficient: recovery MUST leave `queued` jobs queued in their original order (the existing startup sweep in `hostagent/journal.py` rewrites every queued and running job, which would empty the lane on each boot), MUST resolve the formerly-running job to a terminal **`interrupted`** state distinguishable from a tenant-code failure, and MUST settle that job's usage reservation to elapsed GPU-seconds with the remainder released so no quota stays withheld.
