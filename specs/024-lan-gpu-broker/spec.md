...
- **Single GPU-scheduling authority** — `gateway/app/scheduler.py` already exists; it is reduced to a status/routing facade, the host-agent coordinator owns ordering; the job queue is **persisted** (survives restart) and inference has a **bounded burst / job-drain mode** to prevent job starvation (FR-025).
...
**Independent Test**: With a small model resident and budget headroom, send an audio-transcription request whose model also fits; confirm the ASR model becomes co-resident and both modalities serve without evicting each other, and that the combined resident footprint never exceeds live free VRAM.
